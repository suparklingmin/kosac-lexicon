...
lex.match_patterns("나는 정말 행복하다", tok)   # 매칭된 표제어 문자열 목록
...
```bash
kosac analyze "이 영화 정말 좋다"                 # 보기 좋은 JSON
...
`--negation`, `--intensifier`, `--align`, `--compact`. 텍스트 인자를 생략하면
표준입력에서 한 줄당 한 문장을 읽습니다.
