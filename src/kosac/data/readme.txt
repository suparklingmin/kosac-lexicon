...
lexicon.zip에 포함된 csv 파일 여섯 개는 각각 의미 특성 하나에 해당하며, 위에서 서술한 방식으로 얻은 형태소 N-그램 표제어 16,362개(유니그램 3,476개, 바이그램 6,579개, 트라이그램 6,307개)가 가지는 의미 특성값들의 분포로 구성되어 있다. 파일 내에서 각 행은 하나의 N-그램의 감정 특성을 가리킨다. 열에 해당하는 값의 의미는 순서대로 다음과 같다.
	...
	- (의미 특성값): 각 의미 특성값을 가지는 Seed의 개수(절대빈도)

이 패키지 배포본은 절대빈도(카운트)만 저장한다. freq(해당 N-그램을 포함하는 Seed의 개수 = 카운트의 합), max.value(최빈값), max.prop(그 비율)은 파이썬 로더가 카운트에서 계산한다(파일에는 없음). 2016년 최초 공개본은 비율(상대빈도)과 freq·max.value·max.prop을 함께 저장했으며, (절대빈도) = round((비율) × freq) 이다. 절대빈도는 동률을 보존하고 합산·확장이 쉽다.
